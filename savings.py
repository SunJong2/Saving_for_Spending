...
def create_saving(req: SavingCreate, user_id: int = Depends(get_current_user)):
    ...
    with get_db() as conn:
        ...
        cursor.execute(
            "SELECT current_amount, target_amount FROM goals WHERE id = %s",
            (goal_id,)
        )
...
def delete_saving(saving_id: int, user_id: int = Depends(get_current_user)):
    with get_db() as conn:
        ...
        cursor.execute("SELECT goal_id, amount FROM savings WHERE id = %s AND user_id = %s",
        (saving_id, user_id))
        ...
        cursor.execute("SELECT is_completed FROM goals WHERE id = %s", (goal_id,))
        goal = cursor.fetchone()

        if goal[0]:
            raise HTTPException(status_code=400, detail="완료된 목표의 기록은 삭제할 수 없습니다")
        
        cursor.execute("DELETE FROM savings WHERE id = %s", (saving_id,))
        cursor.execute("UPDATE goals SET current_amount = current_amount - %s WHERE id = %s",
        (amount, goal_id))
